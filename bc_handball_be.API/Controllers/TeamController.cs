...
public class TeamController : ControllerBase
{
    ...
    )
    {
        ...
        try
        {
            ...
            var enrichedTeams = teams
                .Select(team =>
                {
                    var dto = teamDtos.FirstOrDefault(t => t.Id == team.Id);
                    return new TeamWithAttributes(team, dto?.Strength ?? 1, dto?.IsGirls ?? false);
                })
                .ToList();
            ...
            {
                variant.GroupCount,
                variant.TotalMatches,
                variant.MinMatchesPerTeam,
                Groups = variant.Groups.Select(group => new
                {
                    group.Id,
                    group.Name,
                    Teams = group
                        .TeamGroups.Select(tg => tg.Team)
                        .Select(team => new TeamGroupAssignDTO
                        {
                            Id = team.Id,
                            Name = team.Name,
                            CategoryId = team.CategoryId,
                            Strength =
                                enrichedTeams.FirstOrDefault(t => t.Team.Id == team.Id)?.Strength
                                ?? 1,
                            IsGirls =
    ...
    public async Task<ActionResult> ImportTeamsFromCsv([FromBody] List<TeamImportCsvDTO> teams)
    {
        ...
        foreach (var dto in teams)
            try
            {
                ...
                var category = await _categoryService.GetByNameAsync(
                    dto.CategoryName,
                    dto.TournamentInstanceId
                );
                if (category == null)
                {
                    skipped++;
                    _logger.LogWarning(
                        "Category not found for name: {CategoryName} in instance {TournamentInstanceId}",
                        dto.CategoryName,
                        dto.TournamentInstanceId
                    );
                    continue;
